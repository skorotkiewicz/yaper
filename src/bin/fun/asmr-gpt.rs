...
impl Trigger {
    ...
    fn play(&mut self, sample_rate: f32) -> f32 {
        ...
        self.noise_burst *= self.decay * self.decay;
        let out = (click * 0.4 + self.noise_burst * 0.6) * self.amp;
        self.amp *= self.decay;
        out
    }

    fn trigger_tap(&mut self, rng: &mut Lcg, sample_rate: f32) {
    ...
    fn trigger_crinkle(&mut self, rng: &mut Lcg, sample_rate: f32) {
    ...
    fn trigger_heartbeat(&mut self, rng: &mut Lcg, sample_rate: f32) {
...
impl Chime {
    ...
    fn play(&mut self, sample_rate: f32) -> f32 {
        if self.target_amp < 0.001 && self.amp < 0.001 {
            return 0.0;
        }
...
impl AsmrEngine {
    ...
    fn next_frame(&mut self) -> (f32, f32) {
        ...
        let br = self.noise.brown().clamp(-2.0, 2.0);
        let pk = self.noise.pink().clamp(-1.5, 1.5);

        let rain_weight = weights[0];
        let brush_weight = weights[1];
        let tingle_weight = weights[2];
        ...
        let drone_lfo =
            (advance_phase(&mut self.drone_lfo_phase, 0.045, self.sample_rate) * TAU).sin() * 0.5
                + 0.5;
        out_l += drone_l * (0.35 + drone_lfo * 0.65);
        out_r += drone_r * (0.35 + drone_lfo * 0.65);
    ...
    fn evolve_scene(&mut self, env: &ReactiveState) {
        ...
        self.drone.freq_base = self.rng.range(42.0, 88.0) + env.slow * 18.0;
    }
...
fn setup_input_stream(host: &cpal::Host, ambient: Arc<AmbientState>) -> Option<Stream> {
    ...
    let stream = match build_input_stream(&input_device, &input_config, ambient) {
        Ok(stream) => stream,
        Err(err) => {
...
fn build_input_stream(
    ...
    match config.sample_format() {
        SampleFormat::I8 => Ok(build_input_stream_typed::<i8>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::I16 => Ok(build_input_stream_typed::<i16>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::I32 => Ok(build_input_stream_typed::<i32>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::I64 => Ok(build_input_stream_typed::<i64>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::U8 => Ok(build_input_stream_typed::<u8>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::U16 => Ok(build_input_stream_typed::<u16>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::U32 => Ok(build_input_stream_typed::<u32>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::U64 => Ok(build_input_stream_typed::<u64>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::F32 => Ok(build_input_stream_typed::<f32>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        SampleFormat::F64 => Ok(build_input_stream_typed::<f64>(
            device,
            &stream_config,
            channels,
            sample_rate,
        ...
        sample_format => Err(format!("unsupported input sample format: {sample_format}").into()),
    }
}
...
fn build_output_stream(
    ...
    match config.sample_format() {
        SampleFormat::I8 => Ok(build_output_stream_typed::<i8>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::I16 => Ok(build_output_stream_typed::<i16>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::I32 => Ok(build_output_stream_typed::<i32>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::I64 => Ok(build_output_stream_typed::<i64>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::U8 => Ok(build_output_stream_typed::<u8>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::U16 => Ok(build_output_stream_typed::<u16>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::U32 => Ok(build_output_stream_typed::<u32>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::U64 => Ok(build_output_stream_typed::<u64>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::F32 => Ok(build_output_stream_typed::<f32>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        SampleFormat::F64 => Ok(build_output_stream_typed::<f64>(
            device,
            &stream_config,
            channels,
            engine_config,
        )?),
        sample_format => Err(format!("unsupported output sample format: {sample_format}").into()),
    }
}
    ...
    T: SizedSample + FromSample<f32> + Send + 'static,
{
    ...
    let err_fn = |err: cpal::StreamError| eprintln!("output stream error: {err}");
    ...
    T: Sample + FromSample<f32>,
{
    for frame in output.chunks_mut(channels) {
        ...
        if channels == 1 {
            ...
            let mono = T::from_sample((out_l + out_r) * 0.5);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let input_stream = setup_input_stream(&host, Arc::clone(&ambient));
    if input_stream.is_none() {
        println!("No microphone active. Running in autonomous mode.");
    }
