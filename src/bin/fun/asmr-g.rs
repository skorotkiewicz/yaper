...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let _input_stream = setup_input_stream(&host, Arc::clone(&ambient), err_tx.clone());
    if _input_stream.is_none() || args.reactivity <= 0.0 {
        println!("No microphone active or reactivity zeroed. Running in autonomous mode.");
    }
    ...
    // Loop continuously looking out for runtime disconnections or crashes.
    // If an error pops up on the stream thread, catch it cleanly here and close down.
    loop {
